...
한 명의 학부생에게 **연구장려금**(방학 펠로우십 기간 포함) 위에 격주 **1:1 PI 멘토링**, **연구에 필요한 AI 에이전트 토큰**(lab 부담), **본인 전용 컴퓨팅 자원**(개인 DGX Spark + Lab GPU 클러스터), 그리고 국제 멘토 네트워크(Brookhaven National Lab, Princeton)와의 직접 협업을 제공합니다.
...
**NeuroX 컨소시움**은 서울대학교(차지욱 교수 총괄)를 중심으로 성균관대·한양대·한국뇌연구원(KBRI)·서울대병원, 그리고 미국 브룩헤이븐 국립연구소(BNL)의 전문가 50여 명이 함께하는 융합 연구센터이며, 위의 *AI Scientist for Brain*은 그 **중장기 목표**입니다. 여러 Brain Foundation Model을 도구처럼 묶어 스스로 가설 → 실험 → 재현을 돌리는 이 자율 뇌과학 AI로, 지금의 신경과학이 겪는 세 한계 — 결과가 잘 재현되지 않고(재현성), 원인을 짚기 어렵고(인과성), 사람마다 다른 뇌를 일반화하기 힘든(개인차) 문제 — 를 대규모 뇌 데이터와 자율 실험으로 정면 돌파하려 합니다. 그래서 ADHD·자폐·치매 같은 질환을 증상이 나타나기 3~5년 전에 미리 선별하고, 우울증 등에서 환자 개인의 뇌 특성에 맞춘 정밀 뇌 자극 치료를 설계하는 것을 지향합니다.
...
> 당신이 40-50세가 되었을 때, 본인의 연구가 세상의 어떤 구체적인 장면을 바꾸어 놓았기를 바라십니까? 그 장면 한 컷을 묘사하고, 그곳에 닿기 위해 앞으로 5년간 풀어야 할 **가장 어려운 문제 하나**와, **그 문제가 지금 풀리지 않는 이유 한 가지**를 적어 주십시오.
...
단, Connectome Lab 자체는 다년간의 연구 트랙 레코드를 가진 연구실이며, lab 출신 학부 연구생 / 대학원생들의 진학·논문 트랙은 [www.connectomelab.com](https://www.connectomelab.com) 에서 확인할 수 있습니다.
